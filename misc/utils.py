...
# Bounding box/pose similarity and association
def oks_iou_18(g, d, a_g, a_d, sigmas=None, in_vis_thre=None):
    if not isinstance(sigmas, np.ndarray):

        sigmas = np.array([.26, .25, .25, .35, .35, .79, .79, .72, .72, .62, .62, 1.07, 1.07, .87, .87, .89, .89, 1.3])/10.0
    vars = (sigmas * 2) ** 2
    yg = g[:, 0]
    xg = g[:, 1]
    vg = g[:, 2]
...
def compute_similarity_matrices(bboxes_a, bboxes_b, poses_a, poses_b):
    ...
    for i, (bbox_a, pose_a) in enumerate(zip(bboxes_a, poses_a)):
        ...
        if poses_a.shape[1] == 18:
            result_pose[i, :] = oks_iou_18(pose_a, poses_b, bbox_area(bbox_a), area_bboxes_b)
        else :
